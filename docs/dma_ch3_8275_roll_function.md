...
1. **rcbios CRT26 (original assembly BIOS pattern — `~/git/rc702-bios`) — does NOT use the roll function, and the 0xFF stop code is *forced by the memory layout*, not by ch3 management.**  `INIT.MAC:549-551` reprograms 8275 PAR2 from `0x98` to `0x59` (`SUB 03Fh` → 25→26 rows, VRTC−1).  `CONOUT.MAC:44` defines `screensize defl 800h-5 = 0x7FB = 2043` (vs `0x7CF = 1999` for 25-row).  `C.MAC:469-507` INTDISPLAY (the 50 Hz CRT ISR) programs ch2 base=`SCREENBASE`/wc=`SCREENSIZE`, **explicitly sets ch3 wc=0 (line 497, "Inactive")**, and that's it.

    The reason the `0xFF` "End of Screen + Stop DMA" code is needed is **not** to manage ch2/ch3 — it's to protect the clock variables that live at fixed high-memory addresses just past where a full 26-row buffer would land (user, 2026-06-14):
    - `SCREENBASE` = `0xF800` (`CONOUT.MAC:32`)
    - `CLOCKADDR` = `0xFFFA`, `CLOCKLOW` = `0xFFFC`, `CLOCKHIGH` = `0xFFFE` (`CDATA.MAC:185-187`)
    - A full 26 × 80 = 2080-byte display starting at `0xF800` would run from `0xF800` to `0xFFFF` — *directly into* CLOCK.  No room.
    - The compromise: 25 full rows (2000 bytes) + 43 bytes of a partial 26th-row "status line" + the `0xFF` stop = 2043 bytes total = `0xF800..0xFFFA-1` — leaves CLOCK alone.
    - That's exactly why `CONOUT.MAC:10` calls it "25 1/2 line" — row 25 holds only ~43 of its 80 character positions before the stop code; literally half a line.

    The 8275 sees the `0xFF` byte mid-stream, interprets it as the "End of Screen + Stop DMA" special character (bits 7:6 = `11`), stops requesting DRQs, and ch2 never reaches TC.  The FF therefore never flips and ch3 is irrelevant — but its `wc=0` programming is still there as a defensive "if something else makes the 8275 request more, ch3 fires for 0 transfers".  This is the inherited idiom that cpnos / C-rcbios picked up.
