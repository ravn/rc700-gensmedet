-- PROBE regression test (target: `make cpnos-todget-test`).
--
-- Boots cpnos, waits for the E> prompt, types `PROBE<CR>`, and
...
-- and the PROBE output (BDOS-version line + payload hex + ASCII).
--
-- Result lands at /tmp/cpnos_todget_result.txt as PASS or FAIL.
-- We declare PASS when the SIO-B contains "PROBE:" lines after the
-- E> prompt (since the slave's CCP prints what PROBE prints).
-- FAIL on timeout or missing PROBE output.
--
...
emu.register_periodic(function()
    ...
    -- Stage 1: see E>, feed PROBE<CR>.
    if stage == 1 then
        local raw = read_siob()
        if raw:find("E>", 1, true) then
            e_prompt_at = t
            e_prompt_siob_len = #raw
            logln(string.format("E> seen at %.2fs (SIO-B len=%d); feeding PROBE<CR>",
                t, e_prompt_siob_len))
            feed("PROBE\r")
            start_stage(2, 30, "wait for PROBE output")
            return
        end
    ...
    -- Stage 2: PROBE prints multi-line output starting "PROBE: ...".
    -- Wait for at least one such line to appear after the E> prompt.
    if stage == 2 then
        local raw = read_siob()
        local tail = raw:sub(e_prompt_siob_len + 1)
        if tail:find("PROBE:", 1, true) then
            -- Give it a couple more seconds to print the full payload.
            if t - stage_at > 8.0 then
                pass(string.format("PROBE ran; %d bytes after E> (see siob.raw)",
                    #tail))
            end
            return
        end
        if t - stage_at > timeout_s then
            fail(string.format("no PROBE: output in %d s after E>; tail=%q",
