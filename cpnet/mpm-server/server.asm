...
@uqcbo	equ	?uqcbo-?base
	...
	db	0,0Ch,0,2,4	; serial number bytes 0..4
	db	0C4h		; serial number byte 5 (site-assigned)
...
; --- Custom extension: Get Time/Date via cpmsim host RTC ports -----
; Reads the z80pack host RTC at I/O ports 25 (CLKCMD) / 26 (CLKDAT),
; per z80pack/iodevices/rtc80.c sub-field encoding:
;   0=sec, 1=min, 2=hr (all BCD), 3=days_lo, 4=days_hi, 5=dom, 6=mon, 7=yr
;
; Reply payload layout (26 bytes):
;   MSG[0..4]: binary 5-byte SCB DAT (days_lo, days_hi, hr, min, sec)
;              -- matches the format expected by ntpdate.asm and the
;                 MP/M XDOS-155 / CP/M-3 SCB date/time layout
;   MSG[5..25]: ASCII "YYYY-MM-DD HH:MM:SS\r\n" -- 21 bytes incl. CR/LF
;              -- for slaves that just want to print and forget
; SIZ = 26 - 1 = 25 per CP/NET encoding.
;
; The ASCII year decoder works for years 2000..2059 (high nibble of
; the year byte fits 10..15 = decimal 100..159 years-since-1900).
; Years pre-2000 or post-2059 will produce a wrong year string but the
; binary 5-byte portion remains correct.
;
; Month is 0-indexed from host's tm_mon (Jan=0); we BCD-add 1 with DAA
; for the human display.

CLKCMD	equ	25
CLKDAT	equ	26

gettod:
	pop	h		; HL = MSGBUF (off=0)
	push	h		; keep MSGBUF on stack for sndbak
	lxi	b,DAT
	dad	b		; HL -> MSG[0]

	; -- Binary 5 bytes: days_lo, days_hi, hr, min, sec --
	mvi	a,3
	call	rdclk
	mvi	a,4
	call	rdclk
	...
	mvi	a,7
	out	CLKCMD
	in	CLKDAT		; A = year byte (packed-decimal)
	call	yr2asc
	mvi	m,'-'
	inx	h
	mvi	a,6
	out	CLKCMD
	in	CLKDAT		; A = month BCD (0-indexed)
	adi	1		; BCD +1 to convert Jan=0..Dec=11 -> 01..12
	daa
	call	bcd2asc
	mvi	m,'-'
	inx	h
	...
	; -- Set SIZ and send back --
	pop	h		; HL = MSGBUF
	lxi	b,SIZ
	dad	b
	mvi	m,25		; SIZ = (5+21)-1 = 25
	jmp	sndbak

; rdclk: read CLKDAT sub-field A, write to (HL), advance HL.
; Trashes A.  Preserves BC, DE.
rdclk:
	out	CLKCMD
	in	CLKDAT
	mov	m,a
...
; bcd2asc: convert BCD byte in A to 2 ASCII digits, write to (HL),
; advance HL twice.  Trashes A; preserves BC, DE.
bcd2asc:
	push	psw
	rrc
	rrc
...
; yr2asc: convert packed-decimal year byte (years-since-1900, encoded
; as (val/10) in high nibble and (val%10) in low; high nibble may be
; 0..15) into 4 ASCII digits "20XX".  Valid for years 2000..2059
; (decoded 100..159).  Writes to (HL), advances HL by 4.
yr2asc:
	push	psw
	mvi	m,'2'
	inx	h
	...
	pop	psw
	push	psw
	ani	0F0h		; high nibble = tens-of-(year-1900)
	rrc
	rrc
	rrc
	rrc
	sui	10		; subtract 10 (we already output "20")
	adi	'0'
	mov	m,a		; tens of year-mod-100
	inx	h
	pop	psw
	ani	0Fh		; low nibble = ones digit
	adi	'0'
	mov	m,a
	inx	h
	ret
...
cpnet:	lxi	h,-@init	;
	...
	xchg
	lxi	h,@srvno+4	; our id from stack
	dad	sp
	mov	a,m
	cpi	10
	jnc	L0321
	...
	jmp	L0323
L0321:	adi	'A'-10
L0323:	stax	d	; set 'x' in "NtwrkQIx"
