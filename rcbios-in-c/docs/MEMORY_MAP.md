...
--------  ------  -------------------------------------------------
0x0000       3    JP WBOOT (warm-boot vector, set by BIOS at boot)
0x0003       1    IOBYTE
0x0004       1    CDISK (current drive)
0x0005       3    JP BDOS (BDOS entry vector)
0x0006–7F  122    (CP/M zero-page scratch, unused)
0x0080      128   DMA buffer (BDOS default transfer area, TBUFF)
0x0100–     –     TPA start (Transient Program Area)

                  ... TPA: 0x0100..0xC3FF = 49920 B = ~48.75 KB ...

0xC400     5632   CCP  (Console Command Processor, 0x1600/2 = 2816 B)
0xCC00     3584   BDOS (0x0E00 B, entry at 0xCC06)
0xDA00      113   BIOS JP table (17 × JP = 0x33 B) + JTVARS (22 B) + ext JT
0xDA71    ~4234   BIOS .text (code)
0xEAFB     ~447   BIOS .rodata (constants, strings, tables)
0xECBA        2   BIOS .data (sentinel marker)
0xECBE     1783   BIOS .bss (uninitialized, zeroed by coldboot)
              ↕   (zeroed BSS ends at 0xF3B4)
0xF500        –   BIOS private stack top (grows down, 0x100 B headroom)
0xF600       36   IVT (18 × 2-byte IM2 vectors, page-aligned at 0xF600)
0xF600        –   Interrupt stack top (grows down into gap below IVT)
0xF680      384   Runtime conversion tables (OUTCON 128 B + INCONV 256 B)
0xF800     2000   Display buffer (80 × 25 chars, DMA ch2 → 8275 CRT)
0xF7D0      –     (display buffer ends at 0xFFCF)
0xFFD0       48   CRT work area (cursor, scroll state, timer)
...
entry vector to point to NDOS. TPA shrinks by ~2 KB.

```
Address   Size    Contents
--------  ------  -------------------------------------------------
0x0000–FF   256   Zero page (same as above)
0x0100–     –     TPA start

                  ... TPA: 0x0100..0xBAFF = 47360 B = ~46.25 KB ...

0xBB00     3072   NDOS.SPR  (CP/NET network BDOS, 0x0C00 B)
0xC700     1280   SNIOS.SPR (network SIO/PIO transport, 0x0500 B)
0xCC00     3584   BDOS (original, overlaid at same address by CPNETLDR)
0xDA00      –     BIOS (unchanged — BIOS JP table, JTVARS, .text, etc.)
...
TPA lost to CP/NET: **2560 B = 2.5 KB** (NDOS 3072 B + SNIOS 1280 B,
minus the original 1792 B CCP area 0xBB00–0xC3FF that moves up).

Actual TPA loss = 0xC400 − 0xBB00 = 0x900 = **2304 B = 2.25 KB**.
...
-----------  -------  -------  ----
.boot        0x0000   0x0000    128 B  boot header (in Track 0 ROM)
...
.text        0xDA71   0x04C5   4234 B  BIOS code
...
.bss         0xECBE   —        1783 B  zeroed at boot (not in binary)
```

Binary (Track 0 image): **5906 B** (LTO, 2026-07-03).
