 ...
 *
 * Each DPB is generated by the DISKDEF macro from seven primitives; the
 * 10 DPB fields (spt, bsh, blm, exm, dsm, drm, al0, al1, cks, off) are
 * derived per the CP/M 2.2 Alteration Guide. See docs/diskdef_macro.md
 * for the full derivation tables. Build-time byte-for-byte identical
 * to the hand-written values they replace.
 *
 * DISKDEF(physical_spt, sector_size, sides, bls, dks, dir, removable, ofs) */

/* BSH: log2(BLS/128). Only BLS in {1024,2048,4096,8192,16384} are valid. */
#define DDF_BSH(bls) \
    ((bls) == 1024  ? 3 : \
     (bls) == 2048  ? 4 : \
     (bls) == 4096  ? 5 : \
...
/* EXM: keyed on BLS and whether dks (==DSM+1) fits one byte. */
...
/* Number of allocation blocks needed for `dir` directory entries:
 * ceil(dir * 32 / bls).  Each entry is 32 bytes. */
#define DDF_DIR_BLKS(dir, bls) (((dir) * 32 + (bls) - 1) / (bls))

/* AL0/AL1: top N bits of a 16-bit word, where N = directory blocks.
 * Bit 15 (MSB of al0) = block 0, bit 0 (LSB of al1) = block 15. */
#define DDF_AL_BITS(dir, bls) ((word)(0xFFFFu - (0xFFFFu >> DDF_DIR_BLKS(dir, bls))))
#define DDF_AL0(dir, bls)     ((byte)(DDF_AL_BITS(dir, bls) >> 8))
#define DDF_AL1(dir, bls)     ((byte)(DDF_AL_BITS(dir, bls) & 0xFF))

/* Full DPB initializer from primitives. */
#define DISKDEF(physical_spt, sector_size, sides, bls, dks, dir, removable, ofs) { \
    .spt = (word)((physical_spt) * ((sector_size)/128) * (sides)),                 \
    .bsh = DDF_BSH(bls),                                                           \
    .blm = (byte)((bls)/128 - 1),                                                  \
    .exm = DDF_EXM(bls, dks),                                                      \
    .dsm = (word)((dks) - 1),                                                      \
    ...
    .al0 = DDF_AL0(dir, bls),                                                      \
    .al1 = DDF_AL1(dir, bls),                                                      \
    .cks = (word)((removable) ? (dir)/4 : 0),                                      \
    .off = (word)(ofs),                                                            \
}

const disk_parameter_block dpb_maxi_128 = DISKDEF(26, 128, 1, 1024, 243,  64, 1, 2);  /* 8" FM  128 B/S (IBM std)  */
const disk_parameter_block dpb_maxi_512 = DISKDEF(15, 512, 2, 2048, 450, 128, 1, 2);  /* 8" DD  512 B/S (MAXI data)*/
const disk_parameter_block dpb_mini_512 = DISKDEF(26, 128, 1, 1024, 243,  64, 1, 0);  /* 5.25" (see note)          */
const disk_parameter_block dpb_maxi_256 = DISKDEF(26, 256, 2, 2048, 472, 128, 1, 0);  /* 8" DD  256 B/S (t0 side 1)*/
/* NOTE: dpb_mini_512 primitives (128 B/S, 26 spt, BLS=1024) match FM track-0
 * layout, not "5.25 DD 512" as the old comment suggested. Preserved verbatim;
 * see tasks/todo.md follow-up for reconciliation. */
