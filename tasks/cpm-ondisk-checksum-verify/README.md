...
Proof that a real C program placed on an RC702 8" DS/DD diskette by
`z88dk-appmake +cpmdisk -f rc700-8dd` is read back by CP/M **byte-for-byte** as
...
## What was verified (2026-08-09, MAME rc702, boot on A:)
...
Results (MAME on-disk output == host reference, bit-for-bit):

| Payload                         | BYTES  | CRC-32     | FNV-1a-32  |
|---------------------------------|--------|------------|------------|
| in-memory `big[]` (40000 B)     | 40000  | `3D6FF5B0` | `DC824845` |
| on-disk `PROG.COM` (48000 B, **3 extents**) | 48000 | `9ABF27DD` | `A22B5087` |

(File checksums depend on the exact `.COM` bytes and change if the program
source changes; the in-memory `big[]` checksums are source-independent. The
values above were captured in MAME with the current `gen_prog.py` source.)

The >32 KB file lays out as EX=0/1/2 (Rc=128/128/116) with **16-bit (word)
block pointers**, blocks 2..25 — matching the reference SW1711 system disk and
confirming the `byte_size_extents=0` fix (z88dk commits `b0e6e7183c` /
`f8ee569914`) at runtime.

The in-memory array checksum is identical under ntvcm and MAME. The on-disk
file checksum intentionally differs between ntvcm (host-file padding) and
MAME/IMD (0xE5 record padding) — expected, since CP/M reads whole 128-byte
records and pads the tail with the disk filler 0xE5.
...
Requires the licensed reference system image `SW1711-I8.imd` for the boot
region (see below). Tool paths default to the macbook layout and are
overridable via env (`WS`, `Z88DK`, `REF_IMD`, `MAME_BIN`, ...).

## The build pipeline

`zcc` drives `appmake` directly via its `-create-app` stage, so compile +
disk-image build is a **single command** (verified: the IMD payload is
byte-identical to a standalone `z88dk-appmake` call; only the IMD header
timestamp differs):
...
| File                 | Role |
|----------------------|------|
| `gen_prog.py`        | emits `prog.c` (the dual-checksum tool); arg = `big[]` size |
| `cpmref.py`          | host reference: extract a file from an rc700-8dd IMD (skew-inverted linear rebuild, word block pointers) and print size + CRC-32 + FNV-1a-32 |
