...
## 2026-06-11 (#110 scope correction)
- #110 / [stop - I want BDOS-105 to work on rcbios+cpnetldr; later make that work under cpnos] / why doesn't ndos3.asm support 105? / [stop] you are heading down the wrong track. Does cpnetldr install something that understands BDOS 105? / [stop] you are heading down the wrong track. Does cpnetldr install something that understands BDOS 105? You can only use CPNET 1.2 not later / i want it written in c / i expect the same todget binary to work identically here as in cpnos
[Reframe: CP/NET 1.2 has no BDOS-105 forwarding; ndos3.asm:504 correctly skips it; TODGET binary already uses BDOS-66/67 which NDOS3 dispatches; same binary should run on rcbios+CPNETLDR unchanged. Built `cpnet/todget_rcbios_test.sh` harness; rewrote finishing-checklist FN-105 section; retitled #110 to "TODGET.COM end-to-end regression via CPNETLDR"; adjusted #111 blocker. New memory rule `feedback_cpnet_12_only.md`.]

## 2026-06-11 (post-wrap-up — restructure cpnet-z80 vs project-side)
- what would you have pushed to cpnet-z80? [3 commits: gettod handler + rebuild-mpm-sys.sh + --install path] / please fork cpnet-z80 and push there [ravn/cpnet-z80 fork already existed; renamed origin->upstream, added fork as origin, pushed 71a4f34] / milestone-merge cpnet-z80 [local merge done at 49459e2, force-push required to push the new structure; warned] / the cpnet-z80 change is about adding the rtc to the server process. This should not change the distribution file which you currently do. Is there a copy of server.asm in the emulator tree? [no — z80pack/cpmsim/srcmpm has netwrkif/bnkxios but no server.asm; user wants project-side relocation] / and restore the original distribution [reset cpnet-z80 to upstream 87f87b6, force-rolled-back ravn/cpnet-z80 fork to match upstream pristine] / please move the copy into the rc700-gensmedet tree instead [moved to cpnet/mpm-server/; script paths updated; end-to-end re-verified PASS] / is z80pack/cpmsim/disks/local the location cpmsim looks for its disks? [no — cpmsim opens disks/drive[a-j].dsk; library/ and local/ are launcher-side conventions one indirection above] / document this and commit / analyse, raise issues and tasks, summarize and commit. [4 new issues #110/#111/#112/#113; timeline + prompts addendum; commit]

## 2026-06-28 (compiler-comparison-corpus: speed column + dcc third compiler + fannkuch clang miscompile)
- [copied work mac->sonnyboy; rebuild] / how are things going / now do compiler comparison / i want relatife paths from the projet root [memory] / you probably need to build z88dk / can you show progress? / has zsdcc been optimized for speed? / i woudl also like one column optimized for speed [added SIZE+SPEED columns to sweep.sh: clang -Oz/-O2, zsdcc --opt-code-size/--opt-code-speed] / look briefly at a [fannkuch -O2 fail], then b / can you add dcc to this comparison, some of the work is already done [built dcc on sonnyboy; new build_dcc_corpus.sh drives dcc->M80/L80 via VirtualCpm.jar(java21)->ticks; dcc_test_main.c harness; size=dccpeep-off, speed=dccpeep-on] / i have vcpm.jar (in cpnet-80; java installed) / search your memory - vcpm.jar done many times [recipe lives in aes256-corpus/build_dcc_aes.sh + cpnos-build VCPM pattern] / ed-fe trap? / doesn't ticks exit if program returns/warm boots? [yes — dcc .COM warm-boots to 0; ticks -end 0 suffices, no ED-FE trap; verify 0xC000 sentinel] / show result [three-way table: llvm wins size on all 5, speed on 3/5; dcc compiles all 5 (pi+licm PASS — dcc exits 1 on recoverable diagnostics but emits correct code; gate on sentinel not exit code; licm caveat: dcc drops the volatile defeat-store)] / analyze, summarize, raise issues as needed, commit / investigate fannkuch clang fail
[VERIFIED: clang -O1/-O2 miscompiles fannkuchredux(7)->0x0000 (should 0x10E4); -Oz/-O0 correct. Middle-end exonerated — -O1 IR passes llc -O0; only llc -O>=1 breaks it -> Z80 BACKEND bug on rotated-loop IR shape. opt-bisect (clang #455 / llc #43) -> branch-folder manifesting pass; -disable-branch-fold alone insufficient (tail-dup/block-placement on same pre-folder MIR also corrupt) -> suspected root in Z80 analyzeBranch/branch-repr. Wrote tasks/clang-fannkuch-O1-backend-miscompile-2026-06-28.md (verified facts + minimal llc reproducer + next steps); routes to llvm-z80/llvm-z80 fork after explain+go-ahead. Commit.]

## 2026-07-16 (elf2rel PR #29 upstream review response)
- jeg har fået tilbagemelding på https://github.com/llvm-z80/llvm-z80/pull/29#issuecomment-4977975269 / [model switch] spørg igen / gem venligst upstream politik for pr's / bemærk det kun er for DENNE upstream, aldrig mame eller z88dk / pretooluse hook må gerne kigge i .cargo / [continued: object crate API research] / jatak, åben i clion / udfyldt / summarize, document and raise issues and tasks, then commit
[Updated upstream filing policy: fixes allowed in PR for llvm-z80/llvm-z80 only when maintainer explicitly asks (not for mame/z88dk). Implemented fix: _BSS area in SDCC_AREAS, section_to_area .bss->_BSS, AreaData logical_len, no zero-byte materialization for SHT_NOBITS. Rewrote test as self-contained via object crate (Architecture::I386 + e_machine patch to 0x1F90; no prebuilt .o). cargo test PASS. Committed 3d0a985, pushed to elf2rel-bss-253-repro. PR reply posted. Post-merge TODO: rebase out local diverging fix commit 284afd1 from ravn/llvm-z80 main after PR merges upstream.]
