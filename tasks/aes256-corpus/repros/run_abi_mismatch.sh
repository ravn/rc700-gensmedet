#!/usr/bin/env bash
# Two-file ABI-mismatch repro for the bug behind ravn/z88dk#5.
#
# Caller TU sees ANSI prototype  -> emits sdcccall 1 register-args.
# Callee TU has K&R definition   -> emits sdcccall 0 stack-args (silent).
# Result: caller and callee disagree on where the first pointer arg lives.
...
cd "$HERE"
mkdir -p build_abi && cd build_abi
cp ../repro_abi_mismatch_caller.c ../repro_abi_mismatch_callee.c .

build_run() {
  local label=$1; shift
  local cflags="$*"

  if ! eval "ZCCCFG=$Z88DK/lib/config PATH=$Z88DK/bin:\$PATH $Z88DK/bin/zcc +z80 -compiler=sdcc $cflags -m -create-app -o ${label} repro_abi_mismatch_caller.c repro_abi_mismatch_callee.c" >${label}.log 2>&1; then
...
k0=d[0xC800]; k31=d[0xC801]; e0=d[0xC802]; d31=d[0xC803]
dec0=d[0xC804]; dec31=d[0xC805]; dec63=d[0xC806]; dec95=d[0xC807]
eoT=d[0xC808]
local_zeroed = (k0==0 and k31==0 and e0==0 and d31==0)
decoy_zeroed = (dec0==0 and dec31==0 and dec63==0 and dec95==0)
if local_zeroed and not decoy_zeroed: status = "PASS"
elif decoy_zeroed and not local_zeroed: status = "BUG-CALLEE-USES-WRONG-PTR"
elif decoy_zeroed and local_zeroed: status = "BOTH-ZEROED"
else: status = f"GARBAGE-local=[{k0:02X},{k31:02X},{e0:02X},{d31:02X}] decoy=[{dec0:02X},{dec31:02X},{dec63:02X},{dec95:02X}]"
print(f'{label:<40s} local=[{k0:02X},{k31:02X},{e0:02X},{d31:02X}] decoy=[{dec0:02X},{dec31:02X},{dec63:02X},{dec95:02X}] eoT={eoT:02X} {status}')
PY
}

echo "Config                                   local              decoy             eoT  status"
echo "---------------------------------------- ------------------ ----------------  ---  ----------"
